...
%span{ class: [ "fa-stack" , "fa-lg" ] }
  - icon_settings.each do | icon |
    = ::TokyoMetro::App::Renderer::Icon::FontAwesome.send( request , icon[ :icon_name ] , icon[ :size ] , spin: base_icon[ :spin ] , pulse: base_icon[ :pulse ] , inverse: base_icon[ :inverse ] ).render
    HAML
  end
