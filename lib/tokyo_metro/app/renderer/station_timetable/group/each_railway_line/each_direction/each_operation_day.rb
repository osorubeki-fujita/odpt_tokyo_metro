...
class TokyoMetro::App::Renderer::StationTimetable::Group::EachRailwayLine::EachRailwayDirection::EachOperationDay < TokyoMetro::Factory::Decorate::MetaClass
  ...
  # @param railway_line_info [RailwayLine] 路線のインスタンス
  ...
  def initialize( request , station_info , railway_line_info , railway_direction , operation_day , station_train_times )
    super( request )
...
%table{ class: [ :station_timetable , operation_day.decorate.css_class ] }
  ...
  def render_header
    h.render inline: <<-HAML , type: :haml , locals: h_locals
%thead{ class: railway_line_info.css_class }
  %tr
    %td{ colspan: 2 , class: :top_header }
      = railway_line_info.decorate.render_in_station_timetable_header
      %div{ class: :main }
  ...
  private
    ...
    # @note
    #   station （駅のインスタンス）に保持されている路線の情報が railway_line_info （路線のインスタンス）と異なる場合は、
    #   railway_line_info を優先し、station と同名の駅に乗り入れている路線の中から railway_line_info と矛盾しない駅のインスタンスを取得する。
  def set_valid_station_info
    unless @station_info.railway_line_info_id == @railway_line_info.id
      @station_info = @station_info.station_infos_including_other_railway_line_infos.find_by( railway_line_info_id: @railway_line_info.id )
    end
  end
