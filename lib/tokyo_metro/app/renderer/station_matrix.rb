...
class TokyoMetro::App::Renderer::StationMatrix < TokyoMetro::Factory::Decorate::MetaClass
  ...
  def initialize( request , railway_lines , make_link_to_railway_line: false , type_of_link_to_station: nil , set_anchor: false , controller_of_linked_page: nil )
    super( request )
...
%div{ id: :station_matrixes }
  - railway_lines.each do | railway_line |
    = railway_line.decorate.matrix.on( request ).render_with_links_to_stations( make_link_to_railway_line , type_of_link_to_station , set_anchor , controller_of_linked_page: controller_of_linked_page )
    HAML
  end
