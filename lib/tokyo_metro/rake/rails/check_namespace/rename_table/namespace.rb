class TokyoMetro::Rake::Rails::CheckNamespace::RenameTable::Namespace
      ...
      ::File.open( filename , "r:UTF-8" ).read.split( /\n/ ).each.with_index(1) do | row_content , row_number |
        ...
        @regexp_list.each_with_index do | regexp , i |
          if regexp === row_content
            @matched_rows << ::TokyoMetro::Rake::Rails::CheckNamespace::RenameTable::Namespace::MatchedRow.new( filename , row_content , row_number , i )
          end
        end
  ...
  def display
    ...
    @matched_rows.group_by( &:regexp_index ).each do | regexp_index , rows |
      ::TokyoMetro::Rake::Rails::CheckNamespace::RenameTable::Namespace::MatchGroup.new( rows , @regexp_list[ regexp_index ] , _filename_length_max ).display( indent )
    end
